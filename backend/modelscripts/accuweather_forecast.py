...
ASOS_CACHE_PATH  = "kmia_asos_21d.csv"   # local cache file
ASOS_DAYS        = 21                     # days of history to fetch
...
def compute(df_asos, asos_label, df_hist24, curr, df_fore, obs_high=None):
    ...
    # ── Forecast path (AccuWeather +1h inputs) ────────────────────────────
    now_ts  = now_utc()
    ...
    if len(df_next) > 0:
        ...
        fore_h_hr = fr["time"].hour
        fore_c    = float(fr["cloud"])      if not np.isnan(fr["cloud"])      else C0
        fore_h    = float(fr["humidity"])   if not np.isnan(fr["humidity"])   else H0
        fore_ws   = float(fr["wind_speed"]) if not np.isnan(fr["wind_speed"]) else W0
        fore_wd   = (float(fr["wind_dir"])  if not np.isnan(fr["wind_dir"])
                     else math.degrees(math.atan2(-U0, -V0)) % 360)
    else:
        fore_time = "+1h"
        fore_h_hr = (H0_hr + 1) % 24
        fore_c = C0; fore_h = H0; fore_ws = W0
        fore_wd = math.degrees(math.atan2(-U0, -V0)) % 360

    fore_u, fore_v = uv_components(fore_ws, fore_wd)

    N_STEPS = int(round(1.0 / SIM_DT))
    def _lp(a, b): return np.linspace(a, b, N_STEPS + 1)

    c_path    = _lp(C0, fore_c)
    h_path    = _lp(H0, fore_h)
    u_path    = _lp(U0, fore_u)
    v_path    = _lp(V0, fore_v)
    dp_path   = np.full(N_STEPS + 1, DP0)
    ...
    return dict(
        ...
        mu_now=mu_now,
        fore_time=fore_time, fore_c=fore_c, fore_h=fore_h,
        fore_ws=fore_ws, fore_s=float(s_path[-1]), fore_dp=DP0,
...
def draw(res, location_key, iteration, next_fetch):
    ...
    L = []
    L.append("=" * W)
    L.append(f"  MIAMI KALSHI TEMPERATURE FORECAST  v6  #{iteration}")
    L.append(f"  LocationKey: {location_key}  |  {MIAMI_LAT}N  {abs(MIAMI_LON):.4f}W")
    L.append(f"  Fit: {res['fit_source']}")
    L.append(f"  Paths: {N_PATHS:,}  |  dt_sim={SIM_DT}h")
    L.append("=" * W)
