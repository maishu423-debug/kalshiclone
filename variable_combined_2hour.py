...
HORIZON_HOURS = 2        # forecast horizon in hours (1 or 2)
...
# AccuWeather forecast blend weight [Phase 1, item 6]. Each hourly VAR(1)
# trajectory state is shrunk toward the deterministic AW forecast by this
# fraction instead of leaving the fetched waypoints unused. This is a fixed
# placeholder — changes.md 2.3 calls for w_h to be selected via walk-forward
# CRPS backtest once an AW forecast archive exists; 0.3 conservatively
# favors the local VAR fit over the external point forecast.
AW_BLEND_WEIGHT = 0.3
...
def sample_var1_path(params_var, x0_vec, n_steps, N=N_PATHS):
    """
    Chain the VAR(1) forward n_steps, retaining every intermediate state
    [Phase 1, items 4-5]: X_0 -> X_1^(m) -> X_2^(m) -> ... -> X_n^(m).

    Previously only the final chained step was kept and then held constant
    for the entire temperature simulation, which effectively applied the
    n-hours-ahead weather state starting at time zero. Returning the full
    per-path trajectory lets the temperature MC use the correct (time-
    varying, interpolated) covariates for each hour of the horizon.

    Returns a list of length n_steps+1:
      traj[0]     : current state x0, broadcast to all N paths
      traj[h]     : per-path dict {key: (N,) array} h hours ahead, h=1..n_steps
    """
    a, B, Sigma = params_var["a"], params_var["B"], params_var["Sigma"]
    k = params_var["k"]

    traj = [{key: np.full(N, x0_vec[i]) for i, key in enumerate(VAR_KEYS)}]

    X_prev = np.tile(x0_vec, (N, 1))   # (N, k) — every path starts at x0
    for _ in range(n_steps):
        x_mean = a + X_prev @ B.T      # (N, k)
        try:
            eps = np.random.multivariate_normal(np.zeros(k), Sigma, size=N)
        except np.linalg.LinAlgError:
            eps = np.random.randn(N, k) * np.sqrt(np.diag(Sigma))
        X_next = x_mean + eps
        state = {}
...
def monte_carlo_temp_vectorised(T0, var_traj, s_path, p,
                                 hour_path=None, N=N_PATHS, dt=SIM_DT,
                                 steps_per_hour=None, dtrend0=0.0):
    """
    Temperature MC that walks the full weather trajectory [Phase 1, items 4-5].

    var_traj : list of length H+1 of per-path covariate dicts {key: (N,) array}
               — one per hourly waypoint (var_traj[0] is the current state
               broadcast to all paths; var_traj[h] is the VAR(1)-simulated
               state h hours ahead — see sample_var1_path). Within each hour,
               covariates are linearly interpolated to the 15-min sub-step
               grid instead of holding the final hour's state constant for
               the whole horizon.
    s_path      : np.array(N_STEPS+1,) — deterministic solar per sub-step
    hour_path   : np.array(N_STEPS+1,) — fractional hour per sub-step
    steps_per_hour : sub-steps per hour (defaults to round(1/dt))
    ...
    As of [Phase 4], each path also uses its own draw of the temp-equation
    coefficients from their sampling covariance (see
    _sample_beta_per_path) rather than the shared point estimate, so
    parameter estimation uncertainty compounds naturally over the horizon
    instead of vanishing entirely.
    """
    T      = np.full(N, float(T0), dtype=np.float64)
    T_max  = T.copy()
    N_STEPS = len(s_path) - 1
    steps_per_hour = steps_per_hour or int(round(1.0 / dt))
    n_hours = len(var_traj) - 1
    ...
    for i in range(N_STEPS):
        # Interpolate the per-path weather state at this sub-step between
        # the two bounding hourly trajectory waypoints [Phase 1, item 5]:
        #   X_{t+q/4}^(m) = (1-q)*X_h^(m) + q*X_{h+1}^(m)
        frac_hour = i / steps_per_hour
        h0 = min(int(frac_hour), n_hours - 1)
        q  = frac_hour - h0
        s0, s1 = var_traj[h0], var_traj[h0 + 1]

        C_arr   = ((1 - q) * s0["cloud"]    + q * s1["cloud"])    / 100.0
        H_arr   = ((1 - q) * s0["rh"]       + q * s1["rh"])       / 100.0
        U_arr   =  (1 - q) * s0["wind_u"]   + q * s1["wind_u"]
        V_arr   =  (1 - q) * s0["wind_v"]   + q * s1["wind_v"]
        DP_arr  =  (1 - q) * s0["dpdt"]     + q * s1["dpdt"]
        DEW_arr =  (1 - q) * s0["dewpoint"] + q * s1["dewpoint"]
        W_arr   = np.sqrt(U_arr**2 + V_arr**2)
        # Onshore/offshore decomposition [Phase 3] — see fit_temp_ou for
        # the coastline-orientation rationale; onshore = -U in this
        # codebase's u/v convention.
...
def compute(df_asos, asos_label, df_hist24, curr, df_fore, obs_high=None):
    ...
    # ── Step C: Sample the full joint-weather trajectory (Change 5) ───────
    # Retain every intermediate hourly state X0 -> X1^(m) -> ... -> X_H^(m)
    # instead of collapsing to only the final step [Phase 1, items 4-5].
    var_traj = sample_var1_path(var_params, x0_vec, HORIZON_HOURS, N=N_PATHS)

    # ── Step D: Blend each hourly VAR state with the AccuWeather forecast ─
    # [Phase 1, item 6]. AW gives a deterministic point forecast for cloud/
    # humidity/wind at each hour; shrink the per-path VAR state toward it
    # rather than leaving the fetched waypoints unused. dpdt and dewpoint
    # aren't in the AW hourly payload, so they stay pure VAR-path values.
    now_ts    = now_utc()
    ...
    for h in range(1, HORIZON_HOURS + 1):
        w = AW_BLEND_WEIGHT
        st = var_traj[h]
        st["cloud"]  = np.clip((1-w)*st["cloud"]  + w*(wp_c[h]*100.0), *VAR_CLIPS["cloud"])
        st["rh"]     = np.clip((1-w)*st["rh"]     + w*(wp_h[h]*100.0), *VAR_CLIPS["rh"])
        st["wind_u"] = np.clip((1-w)*st["wind_u"] + w*wp_u[h],          *VAR_CLIPS["wind_u"])
        st["wind_v"] = np.clip((1-w)*st["wind_v"] + w*wp_v[h],          *VAR_CLIPS["wind_v"])
    ...
    # ── Step E: Temperature MC walking the interpolated trajectory ────────
    # [Phase 1, items 4-5]: covariates vary hour-by-hour along the path
    # instead of the final hour's state being applied from time zero.
    T_end, T_path_max = monte_carlo_temp_vectorised(
        T0, var_traj, s_path, temp_params,
        hour_path=hour_path, N=N_PATHS, dt=SIM_DT, steps_per_hour=STEPS_PER_HOUR,
        dtrend0=dtrend0
    )
    ...
    # Point diagnostics: use the final hourly trajectory state (the
    # +HORIZON_HOURS scenario) for display, same meaning as before.
    var_samples = var_traj[-1]
